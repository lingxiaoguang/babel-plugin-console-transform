import BabelCore, { PluginObj, Visitor } from '@babel/core';
    ...
    return {
        ...
        visitor: {
            CallExpression(path, state) {
                ...
                if (callee.node.type === 'MemberExpression' && (callee.node.object as any).name === 'console') {
                    const methodName = callee.node.property.name as string;
                    if (env === 'production') {
                        if (removeMethods) {
                            const args = path.node.arguments.map(item => (item as any).value);
                        ...
                        if (Object.keys(allStyleMethods).includes(methodName)) {
                            ...
                            path.node.arguments.unshift(
                                ...
                                types.stringLiteral(`${file.opts.filename.slice(process.cwd().length)} (${lineNum}:${columnNum}):`)
                            );
